...
class Hisat(Step):
   ...
   def __init__(self, name, input_files, output_directory, executable , executable_arguments ,\
                genome_reference , known_splice_sites_file,
                single_end_fastq, paired_end_fastqs, p = 1):
      super().__init__(name, input_files, output_directory, executable, executable_arguments)
      self.splice_junction_file = known_splice_sites_file
       ...
       for suffix in suffixes:
          if (not os.path.isfile(ref_base + suffix) ) and\
             (not os.path.isfile(ref_base + suffix + "l")):
              missing_references.append("Couldn't find the HISAT reference: " + ref_base + suffix + " or " +
              ref_base + suffix + "l")
       if len(missing_references) > 0:
           raise StepError("Couldn't find the following Hisat reference(s):\n" +\
   ...
   def prepare(self):
      ...
      self.command = self.executable + " -p " + self.p + " " + self.executable_arguments +\
                    " -x {index} -S {sam_output} " \
                          "--known-splicesite-infile {splice_junction_file} --no-unal "\
                    .format(index = self.genome_reference, sam_output = self.sam_output,
                            splice_junction_file = self.splice_junction_file)
